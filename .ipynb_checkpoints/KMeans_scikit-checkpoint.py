...
def dataset_to_list_points(dir_dataset):
    ...
    with open(dir_dataset, 'rt') as reader:
        for point in reader:
            ...
            points.append(np.asarray(list(map(float, point.split("::"))))) # numpy needs iterable not map objects
    return points
...
def k_means(dataset, num_clusters, max_iterations, init_cluster, tolerance,
    ...
    # Object KMeans
    #versión para 2.7
    kmeans = KMeans(n_clusters=num_clusters, max_iter=max_iterations, init=init_cluster, tol=tolerance, 
                    # n_jobs=num_threads
                   )
...
if __name__ == '__main__':
    ...
    k_means(DATASET1, 7, MAX_ITERATIONS, INITIALIZE_CLUSTERS[0], CONVERGENCE_TOLERANCE, NUM_THREADS)
